...
const TASKS = {
  ...
  'landing-page': { dependencies: ['design-system', 'content-strategy', 'component-library'], priority: 'HIGH' },
  'database-schema': { dependencies: ['nextjs-setup'], priority: 'MEDIUM' },
  'ai-integration': { dependencies: ['api-infrastructure'], priority: 'HIGH' },
  'user-auth': { dependencies: ['database-schema', 'component-library'], priority: 'MEDIUM' },
  'analytics': { dependencies: ['nextjs-setup'], priority: 'LOW' },
  'consulting-integration': { dependencies: ['landing-page'], priority: 'HIGH' },
  'user-flow': { dependencies: ['landing-page', 'user-auth', 'database-schema'], priority: 'MEDIUM' },
  'seo-optimization': { dependencies: ['landing-page'], priority: 'MEDIUM' },
  'deployment-config': { dependencies: ['landing-page', 'api-infrastructure'], priority: 'HIGH' },
  'testing': { dependencies: ['component-library', 'api-infrastructure', 'user-flow'], priority: 'LOW' }
};
...
node agent-workflows/agent-runner.js
  ...
  const terminal = spawn('osascript', [
    '-e', `tell app "Terminal" to do script "cd '${process.cwd()}' && ./.launch-${taskId}.sh"`
  ]);
...
async function launchWave() {
  ...
  if (readyTasks.length === 0) {
    ...
    console.log('All available tasks are either completed, in progress, or waiting for dependencies.\n');
    return;
  }
  ...
  console.log(chalk.gray('💡 Run this command again when agents complete to launch the next wave\n'));
