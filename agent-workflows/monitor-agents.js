...
function displayDashboard() {
  ...
  console.log(
    `[${progressBar}] ${progress.percentage}% (${progress.completed}/${progress.total} tasks)\n`
  );
  ...
  if (activeAgents.length === 0) {
    ...
    activeAgents.forEach(agent => {
      const runtime = Math.floor(
        (Date.now() - new Date(agent.startTime)) / 1000 / 60
      );
      console.log(
        `   • ${chalk.yellow(agent.role)} - ${TASKS[agent.taskId].name} (${runtime} min)`
      );
    });
  }
  console.log('');
  ...
  if (state.inProgressTasks.length === 0) {
    ...
    state.inProgressTasks.forEach(taskId => {
      console.log(
        `   • ${TASKS[taskId].name} - ${TASKS[taskId].estimatedTime}`
      );
    });
  }
  console.log('');
  ...
  if (readyTasks.length === 0) {
    ...
    readyTasks.forEach(task => {
      ...
      console.log(
        `     ${chalk.cyan(`AGENT_ROLE="${task.id}" node agent-workflows/agent-runner.js`)}`
      );
    });
  }
  console.log('');
  ...
  if (activeAgents.length < 4 && readyTasks.length > 0) {
    console.log(
      chalk.green(
        `   • You can run ${4 - activeAgents.length} more agents in parallel`
      )
    );
  }
  ...
  if (progress.percentage > 80 && readyTasks.length > 0) {
    console.log(
      chalk.yellow('   • Nearly done! Deploy remaining tasks to finish')
    );
  }
